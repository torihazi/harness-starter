...
plan(全機能 + sprint 分割)
      │
      ▼  ← 唯一の承認ゲート: 人間が plan 全体を1回だけ承認
      │
  ┌───▼ sprint ループ (承認後・本体セッションが自律で回す) ─────────────────┐
  │  ┌── 機能ループ (sprint 内・逐次に1機能ずつ) ────────────────────┐      │
  │  │  execute ──▶ evaluate ──failing──▶ feedback ──┐              │      │
  │  │     ▲                                          │              │      │
  │  │     └──────────────────────────────────────────┘              │      │
  │  │  passing → features.json 更新 → 機能ごとに commit → 次機能     │      │
  │  └───────────────────────────────────────────────────────────────┘      │
  │  全機能 passing → sprint の PR → go/no-go ───────────────────────────────┼─▶ 次 sprint
  └─────────────────────────────────────────────────────────────────────────┘
```
...
承認済み sprint を優先度順に1つずつ `in_progress` にして回す。各 sprint 内で:
